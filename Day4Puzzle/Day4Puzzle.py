...
def partTwo(input):
    ...
    for row in rows:
        flag = 0
        word_list = row.split()
        if len(word_list) == len(set(word_list)):  #if a row contains a word twice it means it's not valid
            while len(word_list) != 0:
                word = word_list.pop()
                for test_word in word_list:
                    if isAnagram(word,test_word):
                        flag += 1
        else :
            flag += 1
...
inputTest = """kvvfl kvvfl olud wjqsqa olud frc
...
testCase1 = "abcde fghij"
testCase2 = "abcde xyz ecdab"
testCase3 = "a ab abc abd abf abj"
testCase4 = "iiii oiii ooii oooi oooo"
testCase5 = "oiii ioii iioi iiio"
testCase6 = "ab ab"
